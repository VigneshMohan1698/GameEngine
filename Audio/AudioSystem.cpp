...
SoundPlaybackID AudioSystem::StartSound( SoundID soundID, bool isLooped, float volume, float balance, float speed, bool isPaused )
{
	...
	if( channelAssignedToSound )
	{
		...
		channelAssignedToSound->setMode(FMOD_3D_HEADRELATIVE);
...
void AudioSystem::UpdateListener(int listenerIndex, Vec3 listenerPosition, Vec3 listenerForward,  Vec3 listenerUp)
{
	FMOD_VECTOR position, forward, up,velocity;
	SwizzleVector(listenerPosition);
	SwizzleVector(listenerForward);
	SwizzleVector(listenerUp);
...
SoundPlaybackID AudioSystem::StartSoundAt(SoundID soundID,  Vec3 soundPosition, bool isLooped, float volume, float balance, float speed, bool isPaused)
{
	SwizzleVector(soundPosition);
	...
	if (channelAssignedToSound)
	{
		...
		channelAssignedToSound->setPan(balance);
		channelAssignedToSound->setLoopCount(loopCount);
		FMOD_VECTOR pos,Velocity;
		pos.x = soundPosition.x;
		pos.y = soundPosition.y;
		pos.z = soundPosition.z;
...
void AudioSystem::SetSoundPosition(SoundPlaybackID soundPlaybackID, Vec3 soundPosition)
{
	if (soundPlaybackID == MISSING_SOUND_ID)
	{
		return;
	...
	SwizzleVector(soundPosition);
	FMOD::Channel* channelAssignedToSound = nullptr;
	m_fmodSystem->getChannel((int)soundPlaybackID, &channelAssignedToSound);
	FMOD_VECTOR pos, Velocity;
	pos.x = soundPosition.x;
	pos.y = soundPosition.y;
	pos.z = soundPosition.z;
...
void AudioSystem::SwizzleVector(Vec3& vector)
{
	Vec3 temp;
	temp.x = -vector.y;
	temp.y = vector.z;
