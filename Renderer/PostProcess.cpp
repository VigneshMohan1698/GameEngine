...
void PostProcess::InitializeGodRays(ID3D12Device5* device, UINT frameCount, UINT numCallsPerFrame)
{
	...
	shaderBlob = g_theRenderer->m_shaderCompiler->CompileComputeShader("Data/Shaders/PostProcessCS.hlsl");
	...
	g_theRenderer->ThrowIfFailed(hr, "Failed while creating Pipeline State object for post process");
	...
	ID3D12DescriptorHeap* descriptorHeap)
{
	...
	m_postProcessCB->GIColor = g_theRenderer->m_gameValues.GIColor;
	m_postProcessCB->lightPosition = g_theRenderer->m_lightPosition;
	//m_postProcessCB->cameraPosition = g_theRenderer->m_currentCamera.m_position;
	m_postProcessCB->viewMatrix = g_theRenderer->m_currentCamera.GetViewMatrix();
	m_postProcessCB->projectionMatrix = g_theRenderer->m_currentCamera.GetProjectionMatrix();
