...
void Composition::Initialize(ID3D12Device5* device, UINT frameCount, UINT numCallsPerFrame)
{
	...
	GaussianFilterShaderBlob = g_theRenderer->m_shaderCompiler->CompileComputeShader("Data/Shaders/CompositionCS.hlsl");
	...
	g_theRenderer->ThrowIfFailed(hr, "Failed while creating Pipeline State object for Compositor");
...
void Composition::Run(ID3D12GraphicsCommandList4* commandList,ID3D12DescriptorHeap* descriptorHeap, bool denoiserOn)
{
	...
	commandList->SetComputeRootDescriptorTable((int)CompositionRootSignatures::TemporalGBuffer, g_theRenderer->m_denoiser->m_temporalSampler->m_temporalOutput.gpuReadDescriptorHandle);
