...
{
	ResourceManager* resourceManager = g_theRenderer->m_resourceManager;
	commandList->SetComputeRootSignature(m_GIGlobalRootSignature.Get());
	// Copy the updated scene constant buffer to GPU.
	auto GetGBuffers = g_theRenderer->m_resourceManager->m_GpuresourceBuffers;
	//m_irradianceCacheGPUBuffer.CopyFromCPUToGPU(m_frameIndex);
	commandList->SetComputeRootConstantBufferView((UINT)GIGlobalRSParams::SceneConstantBuffer, g_theRenderer->m_sceneCB.GpuVirtualAddress(frameIndex));
	...
	commandList->SetDescriptorHeaps(1, g_theRenderer->m_descriptorHeap.GetAddressOf());
	...
	if (g_theRenderer->m_currentScene == Scenes::Minecraft)
	{
		...
		commandList->SetComputeRootDescriptorTable((UINT)GIGlobalRSParams::VertexAndIndexBuffers, g_theRenderer->m_indexBuffer.gpuReadDescriptorHandle);
		commandList->SetComputeRootShaderResourceView((UINT)GIGlobalRSParams::AccelerationStructure, g_theRenderer->m_topLevelAccelerationStructure->GetGPUVirtualAddress());
	}
	else if (g_theRenderer->m_currentScene == Scenes::Bunny)
	{
		...
		commandList->SetComputeRootDescriptorTable((UINT)GIGlobalRSParams::VertexAndIndexBuffers, g_theRenderer->m_bunnyIndexBuffer.gpuReadDescriptorHandle);
		commandList->SetComputeRootShaderResourceView((UINT)GIGlobalRSParams::AccelerationStructure, g_theRenderer->m_bunnyTlas->GetGPUVirtualAddress());
	}
	...
	dispatchDesc.Width = g_theRenderer->m_dimensions.x;
	dispatchDesc.Height = g_theRenderer->m_dimensions.y;
	dispatchDesc.Depth = 1;
	g_theRenderer->m_dxrCommandList.Get()->SetPipelineState1(m_GIDxrStateObject.Get());
	g_theRenderer->m_dxrCommandList.Get()->DispatchRays(&dispatchDesc);
...
void GlobalIllumination::CreateRootSignatures()
{
	...
	g_theRenderer->SerializeAndCreateRaytracingRootSignature(globalRootSignatureDesc, &m_GIGlobalRootSignature);
}
...
void GlobalIllumination::CreatePipelineStateObject(ID3D12Device5* device, UINT frameCount)
{
	...
	mainShaderBlob = g_theRenderer->m_shaderCompiler->Compile("Data/Shaders/GlobalIllumination.hlsl");
	...
	g_theRenderer->ThrowIfFailed(g_theRenderer->m_dxrDevice->CreateStateObject(raytracingPipeline, IID_PPV_ARGS(&m_GIDxrStateObject)), "Couldn't create DirectX Raytracing state object.\n");
}

void GlobalIllumination::BuildShaderTables(ID3D12Device5* device)
{
	ComPtr<ID3D12StateObjectProperties> stateObjectProps;
	g_theRenderer->ThrowIfFailed(m_GIDxrStateObject.As(&stateObjectProps), "Failed to create properties for shader table");
